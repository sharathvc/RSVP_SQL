...
-- Type your code below:
-- 1st part
select year, sum(title) as number_of_movies 
from movie
group by year;
...
-- Type your code below:
select country,year(date_published) as years ,count(title) as number_of_movies
from movie
...
from movie m inner join ratings a 
on a.movie_id=m.id
group by avg_rating;
...
-- FYI fan is not my favourite movie
...
select languages,count(total_votes) as total_votes
from movie m inner join genre g on g.movie_id=m.id
inner join ratings r on g.movie_id=r.movie_id
where languages='german' or 'italian'
...
+---------------+-------------------+ */
-- Type your code below:
select name as director_name ,count(known_for_movies) as movie_count
from names n inner join genre g on g.movie_id=n.known_for_movies
inner join ratings r on g.movie_id=r.movie_id
where avg_rating > '8'
...
create view cad as
select 
n.name,date_published 
from movie m inner join director_mapping dm on dm.movie_id = m.id
inner join names n on dm.name_id=n.id
order by n.name;

drop view next_cad1;
create view next_cad1 as 
select *,
lag(date_published,1,'2017-06-01') over (partition by name order by date_published) as previous_movie
from cad;
...
inner join names n on dm.name_id = n.id
inner join final2 f1 on f1.name=n.name
